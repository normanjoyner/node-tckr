    ...
    set_controls: function(window, options){
        window.controls = {
            ...
            commands: {
                        ...
                        fs.readFile([root, line[1]].join("/"), function(err, config){
                            if(err)
                                ...
                                try{
                                    ...
                                    window.lines = config;
                                    _.each(config, function(line_number, symbol){
                                        if(line_number > window.current_line)
                                            window.current_line = line_number;
                ...
                d: function(line){
                    line = line.split(" ")[1];
                    if(_.has(window.lines, line)){
                        var curx = window.window.curx;
                        var cury = window.window.cury;
                        window.window.cursor(window.lines[line], 0);
                        window.window.clrtoeol();
                        window.window.refresh();
                        window.window.cursor(cury, curx);
                        delete window.lines[line];
                    }
                },
